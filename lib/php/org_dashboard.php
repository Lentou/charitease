        ...
        //if (isset($_POST["orgName"], $_POST["orgContactName"], $_POST["orgAddress"], $_POST["orgFoundingDate"], $_POST["orgPhone"])) {
          ...
          if (isset($_FILES["profile_pic"])) {
            ...
            if ($get_pic->num_rows > 0) {
                $update_pic = $db->query("UPDATE `tblimages` SET image_name = '$imageName', image_type = '$fileType', image_date = '$imageData' WHERE table_id = '$org_id'");
            } else {
    ...
    if (isset($_POST["tline_status"])) {
        ...
        if ($tline == "edit") {
            ...
            if ($editEventType == "event") {
                $editEventStartDate = isset($_POST["editEventStartDate"]) ? $_POST["editEventStartDate"] : NULL;
                $editEventEndDate = isset($_POST["editEventEndDate"]) ? $_POST["editEventEndDate"] : NULL;
                $editCurrentInkind = isset($_POST["editCurrentInkind"]) ? $_POST["editCurrentInkind"] : 0;
                $editTargetInkind = isset($_POST["editTargetInkind"]) ? $_POST["editTargetInkind"] : 0;
                $editCurrentFunds = isset($_POST["editCurrentFunds"]) ? $_POST["editCurrentFunds"] : 0 ;
                $editTargetFunds = isset($_POST["editTargetFunds"]) ? $_POST["editTargetFunds"] : 0;
            } else {

...
<?php
  ...
  <?php 
      ...
      $did = $_SESSION['id'];
      $dnr = $db->query("SELECT * FROM `tbldonors` WHERE donor_id = '$did'");
      if ($dnr) {
        $dd = mysqli_fetch_assoc($dnr);
        $dadd = $dd['donor_address'];
      }

      $geocode_url = 'https://nominatim.openstreetmap.org/search?format=json&q=' . urlencode($dadd);
      ...
      if (!empty($geocode_data)) {
        $dlatitude = $geocode_data[0]['lat'];
        $dlongitude = $geocode_data[0]['lon'];
      }  
          ...
          while ($row = mysqli_fetch_assoc($result)) {
              ...
              if (!empty($geocode_data)) {
                  ...
                  $org_charities[] = [
                    ...
                    'latitude' => $charity_latitude,
                    'longitude' => $charity_longitude
                  ];
              }
          }
      }

      foreach ($org_charities as $char) {
        $long = $char['longitude'];
        $lat = $char['latitude'];
        $chari = $char['name'];
        $chari_add = $char['address'];

        $distanced = calculateDistance($dlatitude, $dlongitude, $lat, $long);

        if ($distanced < $nearest_distance) {
          $nearest_charity = $chari;
          $nearest_address = $chari_add;
          $nearest_distance = $distanced;
        }
      }
  ?>
              ...
              <div class="column">
                  <div class="box">
                      ...
                      <ul style="font-family:'Times New Roman', Times, serif;">
                        ...
                        <li>You can hover and see the name, address and distance of Charitable Organization</li>
                  ...
                  <div class="box">
                    <p style="font-family:'Times New Roman', Times, serif;">
                          <strong>Nearest Charity Organization: </strong>
                          <br>
                          <?php echo $nearest_charity . " [" . $nearest_distance . " km]"; ?> <br>
                          <?php echo $nearest_address; ?>
                      </p>
                  </div>
              </div>
          </div>
...
if (!empty($geocode_data)) {
  ...
  echo '<script>
      ...
      function handleMarkerHover(event) {
          ...
          var charityDistance = feature.get("charityDistance");

          var popupContent = document.getElementById("popup-content");
          popupContent.innerHTML = "<strong>" + charityName + "</strong><br>" + charityAddress + "<br>Distance: " + charityDistance + " km";
      ...
      function handleMarkerClick(event) {
          ...
          if (charityId == "id") {
              ...
              window.location.href = "timeline.php?oid="  + charityId;
          }
      }
  ...
  $query = "SELECT org_id, org_name, org_address FROM tblorgs WHERE is_approved = '1'";
  $result = mysqli_query($conn, $query);

          ...
          if ($row[$pw] == $password) {
            if (array_key_exists("account_type", $row)) {
              if ($row["account_type"] == $role) {
                ...
                if ($row["account_type"] == "donor") {
                  $is_approve = $db->query("SELECT * FROM `tbldonors` WHERE donor_id = '$user_id' AND is_approved = '1'");
                } else if ($row["account_type"] == "charity") {
                  $is_approve = $db->query("SELECT * FROM `tblorgs` WHERE org_id = '$user_id' AND is_approved = '1'");
                }

                if (mysqli_num_rows($is_approve) == 1) {
                  ...
                  $_SESSION['status_text'] = "You are not verified yet!";
          ...
          <div class="column is-4 is-offset-4">
              ...
              <p class="subtitle" id="message" name="message">Please login to proceed</p>
                <form method="POST">
                  <div class="field">
                    <div class="control">
                      <input class="input is-medium" type="email" placeholder="Your Email" autofocus="" name="email" autocomplete="off">
                    </div>
                  </div>
                  <div class="field">
                    <div class="control">
